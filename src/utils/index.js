...
export function formattedPercent(percent, useBrackets = false) {
  ...
  if (percent < 0.0001 && percent > 0) {
    return (
      <Text fontWeight={500} color="#00e678">
        {'< 0.0001%'}
      </Text>
    )
  }
  ...
  if (fixedPercent > 0) {
    if (fixedPercent > 100) {
      return <Text fontWeight={500} color="#00e678">{`+${percent?.toFixed(0).toLocaleString()}%`}</Text>
    } else {
      return <Text fontWeight={500} color="#00e678">{`+${fixedPercent}%`}</Text>
    }
  } else {
